...
class AgentToolRuntime:
    ...
    def _edit_file(self, args: Dict[str, Any]) -> ToolExecutionResult:
        ...
        for index, edit in enumerate(edits):
            ...
            if not old_text:
                failed.append(
                    {
                        "index": index,
                        "error": "edit_file requires old_text",
                        "old_text": old_text,
                        "new_text": new_text,
                    }
                )
        ...
        if failed and not succeeded:
            # All edits failed — file state unchanged, no content update
            return ToolExecutionResult(
                ok=False,
                result={
                    "error": f"All {len(failed)} edit(s) failed",
                    "failed": failed,
                    "succeeded": [],
                },
                summary={
                    ...
                    "failed": [
                        {
                            "index": f["index"],
                            "error": f["error"],
                            "old_text": summarize_text(f["old_text"], 140),
                        }
                        for f in failed
                    ],
                },
        ...
        if failed:
            # Some edits succeeded, some failed — commit successful edits
            return ToolExecutionResult(
                ok=False,
                result={
                    "error": f"{len(failed)} of {len(edits)} edit(s) failed; "
                    f"{len(succeeded)} succeeded and were applied",
                    "failed": failed,
                    "succeeded": succeeded,
                    "details": {
                        "diff": diff_info["diff"],
